...
namespace TradingBot
{
	public class Algorithm
	{
		...
		{
			string symbol = Globals.QuoteCurrency + Globals.BaseCurrency;

			//GetAccountInformation
			var accInfo = API.GetAccountInformation();

			//Check account funds for both currencies
			AssetBalance Currency1 = (from coin in accInfo.Balances where coin.Asset == Globals.QuoteCurrency select coin).FirstOrDefault();
			var freeQuoteCurrency1 = Currency1.Free;

			AssetBalance Currency2 = (from coin in accInfo.Balances where coin.Asset == Globals.BaseCurrency select coin).FirstOrDefault();
			var freeBaseCurrency2 = Currency2.Free;
			...
			if (marketOrder == null)
			{
				...
				{
					Console.WriteLine(actualLastTrade.Qty + " of " + Globals.QuoteCurrency + " was previously bought for " + actualLastTrade.Price);
				}
				else if (actualLastTrade.isBuyer == false)
				{
					Console.WriteLine(actualLastTrade.Qty + " of " + Globals.QuoteCurrency + " was previously sold for " + actualLastTrade.Price);
				}
			}
			else
			{
				...
				{
					Console.WriteLine(newLastTrade.Qty + " of " + Globals.QuoteCurrency + " was bought for " + newLastTrade.Price);
				}
				else if (marketOrder.Side == OrderSides.SELL)
				{
					Console.WriteLine(newLastTrade.Qty + " of " + Globals.QuoteCurrency + " was sold for " + newLastTrade.Price);
				}
			}
